...
async def get_dashboard(
    ...
    devis_expirant_result = await db.execute(
        select(Devis)
        .options(joinedload(Devis.client))
        .where(
            Devis.id_user == current_user.id,
            Devis.statut.in_(["brouillon", "envoyé"]),
            (Devis.date_devis + func.cast(
                func.concat(Devis.nb_jours_validite, ' days'),
                type_=Devis.date_devis.type
    ...
    try:
        devis_expirant = devis_expirant_result.unique().scalars().all()
    except Exception:
        # Fallback: calcul en Python si l'expression SQL n'est pas supportée
        devis_expirant = []
